...
type CommonBuddy = Pick<Friend, 'name'>; // 或者更具体的共有属性

function intersection(friends: Friend[], colleagues: Colleague[]): CommonBuddy[] {
    let result: CommonBuddy[] = [];
    friends.forEach((friend) => {
        const colleague = colleagues.find((col) => col.name === friend.name);
        if (colleague) {
            result.push({ name: friend.name }); // 将共有的属性添加到结果数组
        }
    });
    return result;
}
